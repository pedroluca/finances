...
switch ($_SERVER['REQUEST_METHOD']) {
  ...
  case 'DELETE':
    ...
    try {
      // 1. Buscar se o item é de parcelamento
      $stmt = $pdo->prepare('SELECT installment_group_id FROM invoice_items WHERE id = :id');
      $stmt->execute(['id' => $id]);
      $row = $stmt->fetch(PDO::FETCH_ASSOC);

      if ($row && !empty($row['installment_group_id'])) {
        // 2. Deleta todas as parcelas do grupo
        $stmt = $pdo->prepare('DELETE FROM invoice_items WHERE installment_group_id = :group_id');
        $stmt->execute(['group_id' => $row['installment_group_id']]);
        echo json_encode([
          'success' => true,
          'deleted_group' => $row['installment_group_id'],
          'deleted_all_installments' => true
        ]);
      } else {
        // 3. Deleta só o item individual
        $stmt = $pdo->prepare('DELETE FROM invoice_items WHERE id = :id');
        $stmt->execute(['id' => $id]);
        echo json_encode(['success' => true, 'deleted_all_installments' => false]);
      }
    } catch (PDOException $e) {
      http_response_code(500);
      echo json_encode([
